...
def convert_bytes_to_dataframe(byte_data, encoding='utf-8', **kwargs):
    """
    Converts a bytes object to a Pandas DataFrame.
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        st.image(image) #for show
        st.session_state.uploaded_image = image
    
    if st.session_state.uploaded_image is not None:
        response = st.session_state.model.generate_content(["Tell me about this image",
    ...
    if uploaded_file is not None:
        bytes_data = uploaded_file.getvalue()
        df = convert_bytes_to_dataframe(bytes_data, delimiter=',', names=['col1', 'col2'])
        st.write(df)
        st.session_state.df_ = {'col1':'abc'}
    
    if st.session_state.df_ is not None:
        response = st.session_state.model.generate_content(["Tell me about this data",
