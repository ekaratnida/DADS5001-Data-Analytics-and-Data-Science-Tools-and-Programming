...
with st.sidebar:
    ...
    import io
    uf = st.file_uploader("Choose a file.")
    bytes_data: bytes = None
    if uf is not None:
        bytes_data = uf.getvalue()
        img = Image.open(io.BytesIO(bytes_data))
        st.image(img)
        response = st.session_state.model.generate_content(["Tell me about the image",img])
    ...
    uf_csv = st.file_uploader("Choose a csv.")
    if uf_csv is not None:
        bytes_data = uf_csv.getvalue()
        df = convert_bytes_to_dataframe(bytes_data, delimiter=',')
        st.write(df)
        df_dict = df.to_dict(orient='records')
        response = st.session_state.model.generate_content("Tell me about the dataframe "+str(df_dict))
        st.write(response.text)
