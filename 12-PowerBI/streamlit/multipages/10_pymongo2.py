...
def get_db(db_name):
    ...
    return client[db_name] # database

def get_collection(db_name, collectionName):
    db = get_db(db_name)
    return db[collectionName] # table
...
try:
    movies = get_collection('sample_mflix','movies')
...
with update_col1:
    with st.container(border=True):
        ...
        if "parasite_id" in st.session_state:
            if st.button("Update Year (one) to 2019"):
                try:
                    movies = get_collection('sample_mflix','movies')
...
with update_col2:
    with st.container(border=True):
        ...
        if st.button("Update All (many) Parasite Docs"):
            try:
                movies = get_collection('sample_mflix','movies')
...
with update_col3:
    with st.container(border=True):
        ...
        if st.button("Reset Year (many) to 2020"):
            try:
                movies = get_collection('sample_mflix','movies')
                update_result = movies.update_many(
                    {"title": "Parasite"},
